...
def main():
    ...
    save_directory = './Himanis/features/Junclets_aug/size_' + sys.argv[1] + '/'  # Directory to save features in
    file_dir = '../Himanis/Junclets_aug/'

    if not os.path.exists(save_directory):
        os.makedirs(save_directory)

    start_year = 1305             # Earliest key year/class
    n_classes = 4                 # Number of classes
    h = size * n_classes          # Size of the codebook
    classes = [1305, 1311, 1319, 1326]  # List of key years/classes

    codebook = []

    # Obtaining the codebook by concatenating sub-codebooks for all classes
    for year in tqdm(classes):
        dir_name = './Himanis/500_weights/' + str(year) + '/'
        if year == start_year:
